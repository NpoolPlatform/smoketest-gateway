    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/smoketest-gateway/k8s/02-smoketest-gateway.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/smoketest-gateway/k8s/02-smoketest-gateway.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/smoketest-gateway/k8s/02-smoketest-gateway.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
